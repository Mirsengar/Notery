               ...
               void onSuccess ( int statusCode , Header[] headers , byte[] responseBody ) {
                    ...
                    try {
                         ...
                         if ( jsonObject.has ( "error" ) ) {
                              ...
                              LoginObject loginObject = new Gson ( ).fromJson ( response ,
                                                                                LoginObject.class );
                              if ( loginObject.getState ( ).equals ( ROUTER.SUCCESS ) ) {
                                   String name =
                                             loginObject.getUserObject ( ).getFNAME ( ) + " " + loginObject.getUserObject ( ).getLNAME ( );
                                   app.t ( "Welcome " + name , app.ToastType.SUCCESS );
                                   spref.get ( ).edit ( )
                                        .putInt ( "nightMode" ,
                                                  loginObject.getSettingsObject ( ).getNightMode ( ) )
                                        .putString ( "font" ,
                                                     loginObject.getSettingsObject ( ).getFont ( ) )
                                        .putInt ( "bgColor" ,
                                                  loginObject.getSettingsObject ( ).getBgColor ( ) )
                                        .putInt ( "textColor" ,
                                                  loginObject.getSettingsObject ( ).getTextColor ( ) )
                                        .putInt ( "fontSize" ,
                                                  loginObject.getSettingsObject ( ).getFontSize ( ) )
                                        .putInt ( "avatarID" ,
                                                  loginObject.getSettingsObject ( ).getAvatarObject ( ).getId ( ) )
                                        .putString ( "name" ,
                                                     loginObject.getSettingsObject ( ).getAvatarObject ( ).getName ( ) )
                                        .putString ( "image" ,
                                                     loginObject.getSettingsObject ( ).getAvatarObject ( ).getImage ( ) )
                                        .apply ( );
                                   app.l ( ROUTER.SESSION + loginObject.getUserObject ( ).getSESSION ( ) );
                                   app.l ( ROUTER.USER_ID + loginObject.getUserObject ( ).getUSER_ID ( ) );
                                   spref.get ( ).edit ( )
                                        .putString ( ROUTER.SESSION ,
                                                     loginObject.getUserObject ( ).getSESSION ( ) )
                                        .putString ( ROUTER.INPUT_EMAIL ,
                                                     email.getText ( ).toString ( ) )
                                        .putInt ( ROUTER.USER_ID ,
                                                  loginObject.getUserObject ( ).getUSER_ID ( ) )
                                        .putString ( ROUTER.INPUT_FNAME ,
                                                     loginObject.getUserObject ( ).getFNAME ( ) )
                                        .putString ( ROUTER.INPUT_LNAME ,
                                                     loginObject.getUserObject ( ).getLNAME ( ) )
                                        .putInt ( ROUTER.INPUT_SEX ,
                                                  loginObject.getUserObject ( ).getSEX ( ) )
                                        .apply ( );
